...
- Regole cursore: l&#39;automazione personalizzata e la guida risiedono in `.cursor/rules/*.mdc`. Esempi: `.cursor/rules/docs-lint.mdc` (processo lint), `.cursor/rules/generate-release-notes.mdc` (punta all&#39;abilità note sulla versione durante la modifica di `help/user-guide/release-notes.md`). Segui questi passaggi per le attività automatizzate.
- Competenze agente: i flussi di lavoro attività sono attivi in `.cursor/skills/<name>/SKILL.md`. Esempio: `.cursor/skills/generate-release-notes/SKILL.md` (procedura completa per le note sulla versione di GenStudio; utilizzare con `examples.md` e `reference.md` nella stessa cartella).
- Nomi di file e contenuti anteriori:
   - Note sulla versione: mantieni elemento Experience League esistente su `help/user-guide/release-notes.md` (vedi `.cursor/skills/generate-release-notes/reference.md#frontmatter`); la regola thin `.cursor/rules/generate-release-notes.mdc` viene allegata automaticamente quando il file viene modificato.
...
- Non aggiungere mai Jira ID, collegamenti interni o riferimenti ai soli documenti pubblici all’interno dell’azienda. Vedere `.cursor/skills/generate-release-notes/SKILL.md` (sezione **Contenuti non consentiti**).
