...
Quando si seleziona un [!DNL Experience] in MFE e si fa clic sul pulsante **[!UICONTROL Usa]**, è necessario richiamare una funzione di callback per trasmettere i dettagli dell&#39;elemento [!DNL Experience] selezionato.

Il callback fornisce una stringa HTML che rappresenta [!DNL Experience]. Se l&#39;esperienza include risorse (ad esempio immagini e video), gli elementi HTML, come `<img>` e `<video>`, includeranno gli attributi `src` che contengono URL prefirmati per queste risorse. Questi URL prefirmati hanno una scadenza di 30 minuti e [!DNL Experience] devono essere utilizzati dall&#39;applicazione client in tale periodo.

### Schema di output

Il callback `onSelectionConfirmed` riceve un array di [!DNL Experience] oggetti con la seguente struttura:

| Proprietà | Tipo | Descrizione |
|----------|------|-------------|
| `id` | `string` | Identificatore univoco per [!DNL Experience] (formato URN). |
| `content` | `string` | Stringa HTML con codifica Base64 che rappresenta [!DNL Experience]. |
| `experienceFields` | `object` | Coppie chiave-valore di campi modificabili in [!DNL Experience]. Ogni campo in `experienceFields` contiene: <ul><li>`fieldName`: nome/identificatore del campo.</li><li>`fieldValue`: il contenuto/valore effettivo del campo.</li></ul> |
| `metadata` | `object` | Metadati sull&#39;esperienza (ad esempio `channel`, `externalTemplateMetadata`, `externalAssetMetadata` ecc.) |
| `aspectVariants` | `array` | Matrice di varianti di proporzioni per [!DNL Experience]. Ogni variante contiene: <ul><li>`aspectKey`: identificatore univoco per la variante.</li><li>`aspectMetadata`: metadati, inclusi `channel` e `aspectRatio`.</li><li>`content`: stringa HTML con codifica Base64 per queste specifiche proporzioni.</li></ul> |

### Esempio di JSON

Di seguito è riportato un blocco JSON di esempio che mostra gli schemi.
